...
`POST /commands/scalp-start`, `GET /scalp/{session_id}`, `POST /scalp/scheduler/tick` et `POST /commands/scalp-stop/{session_id}` exposent le socle scalping paper-only cote API.
