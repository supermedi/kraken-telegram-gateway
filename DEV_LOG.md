...
- Kraken Futures: authenticated REST signing, private request preparation, read-only `/derivatives/api/v3/accounts` balance lookup exposed through Telegram `/balance`/`/solde` and API `GET /balance` with optional filters, read-only `/derivatives/api/v3/fills` recent-fill lookup for live scalp entry reconciliation, a local-first/public-fallback instrument metadata provider, a metadata cache validator CLI, safe entry/target limit-order payload boundaries, live order POST submission to `/derivatives/api/v3/sendorder`, and live order cancellation via `/derivatives/api/v3/cancelorder` are implemented behind the existing live gates.
...
- Verification baseline: `python3 -m pytest -q` was last reported passing with 141 tests on 2026-08-17.
- Scalping mode: V1 foundation implemented as a separate paper-first session mode with persistent `ScalpSession`/`ScalpTrade`/`ScalpSignal` tables, `/scalp_start`, `/scalp_status`, `/scalp_stop`, `/scalp_report`, `/scalp_tick_kraken`, `/scalp_sync_fills`, API endpoints including `GET /scalp/{session_id}/report` and `POST /scalp/{session_id}/sync-fills`, multi-minute `max_hold`, synthetic market-data runner, injectable active-session scheduler, Kraken Futures public WebSocket snapshot collection, opt-in FastAPI background loop, offline deterministic replay CLI for one or more JSON/JSONL/CSV snapshot files, common historical book export aliases, nested order-book export levels such as `bids`/`asks` and Binance `b`/`a`, OHLCV/kline replay inputs converted into synthetic paper-only top-of-book snapshots, raw public Kraken Futures WebSocket book/ticker message replay, multi-file replay summaries, and paper metrics for winrate, gross/net PnL, estimated fees, max drawdown, rejected signals, close reasons, and stop reason. Experimental `mode=live` can submit one Kraken limit entry per signal only when `SCALP_LIVE_ENABLED=true`, existing live gates are open, the pair is allowed, and amount is under `SCALP_LIVE_MAX_AMOUNT_USDC`; submitted live scalp entries can now be reconciled from read-only recent fills, but automatic live exits are not implemented yet.
...
3. Add additional exchange/download-specific historical-data adapters beyond raw Kraken WebSocket exports, nested order-book rows, and OHLCV/kline rows if a real export format still fails replay loading.
4. Add continuous Kraken account-event polling/webhook tracking and automatic live scalp reduce-only exit planning after fill reconciliation.
...
- Chose Next Queue item 3 and added another historical-data adapter slice for coarse paper validation from common candle exports.
- Extended `kraken-scalp-replay` row loading to accept CCXT-style OHLCV arrays `[timestamp, open, high, low, close, volume]`, OHLCV/kline mappings, and nested Binance-style `k` payloads.
- OHLCV/kline rows are converted into synthetic paper-only top-of-book snapshots around the close price with optional `spread_bps`, optional `volumeRatio`, and taker-buy volume aliases for imbalance when available.
- Preserved existing canonical bid/ask rows, nested order-book rows, and raw Kraken Futures WebSocket replay handling.
- Checked the top Docker/VPS validation item again locally, but Docker is not installed in this cron environment; VPS/GHCR validation remains pending.
...
Tests: `python3 -m pytest tests/test_scalping.py -q` -> 14 passed. `python3 -m compileall -q kraken_telegram_gateway` -> OK. `python3 -m pytest -q` -> 136 passed, 1 known Starlette warning. `docker version` -> unavailable (`docker: command not found`).

Commit local: `Accept OHLCV scalp replay inputs` (hash final reported in the cycle report).
...
- Chose Next Queue item 3 and completed another historical-data adapter slice for longer paper validation.
- Extended offline scalp replay loading to accept nested order-book export rows with `bids`/`asks`, `bid_levels`/`ask_levels`, and Binance-style `b`/`a` levels.
- Added millisecond epoch timestamp handling for exchange fields such as `E`, while preserving canonical snapshot rows and raw Kraken Futures WebSocket replay handling.
- Kept Kraken safety guardrails unchanged: no live-trading flag, dry-run default, credentials, Kraken network-order path, or live execution behavior was changed.

Files changed: `kraken_telegram_gateway/gateway/scalp_replay.py`, `tests/test_scalping.py`, `README.md`, `SCALPING_MODE.md`, `DEV_LOG.md`.

Tests: `python3 -m pytest tests/test_scalping.py -q` -> 12 passed. `python3 -m compileall -q kraken_telegram_gateway` -> OK. `python3 -m pytest -q` -> 134 passed, 1 known Starlette warning.

Commit local: `Accept nested order book replay inputs`.

### 2026-08-17 18:33 UTC - Gated Scalp Live Entry

- User explicitly requested live scalping for a small-amount test.
- Added experimental `mode=live` support for scalp sessions behind `SCALP_LIVE_ENABLED=true`, the existing Kraken live gates, allowed-pair validation, and `SCALP_LIVE_MAX_AMOUNT_USDC`.
- Extended the Kraken client with a scalp entry limit-order submission path that reuses existing instrument metadata sizing and signed `/sendorder` handling.
- Updated the Kraken scalp scheduler so active paper sessions remain simulated while active live sessions can submit one entry order when a signal passes.
- Added `live_active`, `live_submitted`, and `live_blocked` state tracking plus `external_order_id` persistence for scalp trades, with an idempotent SQLite column migration.
- Kept automatic live exits and fill tracking out of scope; these remain blocked until account-event/fill tracking is implemented.

Files changed: `kraken_telegram_gateway/gateway/config.py`, `kraken_telegram_gateway/gateway/db.py`, `kraken_telegram_gateway/gateway/models.py`, `kraken_telegram_gateway/gateway/schemas.py`, `kraken_telegram_gateway/gateway/kraken.py`, `kraken_telegram_gateway/gateway/service.py`, `kraken_telegram_gateway/gateway/telegram.py`, `kraken_telegram_gateway/gateway/app.py`, `tests/test_scalping.py`, `tests/test_telegram.py`, `README.md`, `SCALPING_MODE.md`, `DEPLOYMENT.md`, `DEV_LOG.md`.

Tests: targeted scalp/API/Telegram tests -> 17 passed, 1 known Starlette warning. `python3 -m pytest -q` -> 133 passed, 1 known Starlette warning.

### 2026-08-17 16:48 UTC - Raw Kraken WebSocket Replay Input

- Chose Next Queue item 3 and completed a Kraken-specific historical-data adapter slice for longer paper validation.
- Extended offline scalp replay loading so JSON/JSONL exports of public Kraken Futures `book_snapshot`, `book`, and `ticker_lite` messages are converted through the existing `KrakenFuturesBook` adapter into `MarketSnapshot` rows.
- Preserved generic CSV/JSON snapshot input and existing historical alias support.
- Kept Kraken safety guardrails unchanged: no live-trading flag, dry-run default, credentials, Kraken network-order path, or live execution behavior was changed.

Files changed: `kraken_telegram_gateway/gateway/scalp_replay.py`, `tests/test_scalping.py`, `README.md`, `SCALPING_MODE.md`, `DEV_LOG.md`.

Tests: `python3 -m pytest tests/test_scalping.py -q` -> 10 passed. `python3 -m compileall -q kraken_telegram_gateway` -> OK. `python3 -m pytest -q` -> 131 passed, 1 known Starlette warning.

Commit local: `Accept raw Kraken WebSocket replay inputs`.
...
- Chose Next Queue item 3 and completed the multi-session replay summary slice for longer paper validation.
- Extended `kraken-scalp-replay` so `--snapshots` accepts multiple JSON, JSONL, or CSV files.
- Preserved the single-file output shape for compatibility; multi-file runs now emit `runs` plus an aggregate `summary`.
- Aggregated replay count, closed/open trades, wins/losses, winrate, gross/net PnL, estimated fees, average net PnL per replay, worst per-replay drawdown, rejected signals, close reasons, and stop reasons.
- Kept Kraken safety guardrails unchanged: no live-trading flag, dry-run default, credentials, Kraken network-order path, or live execution behavior was changed.

Files changed: `kraken_telegram_gateway/gateway/scalp_replay.py`, `tests/test_scalping.py`, `README.md`, `SCALPING_MODE.md`, `DEV_LOG.md`.

Tests: `python3 -m pytest tests/test_scalping.py -q` -> 8 passed. `python3 -m compileall -q kraken_telegram_gateway` -> OK. `python3 -m kraken_telegram_gateway.gateway.scalp_replay --help` -> OK. `python3 -m pytest -q` -> 129 passed, 1 known Starlette warning.

Commit local: `Add multi-file scalp replay summaries`.

### 2026-08-17 13:44 UTC - Offline Scalp Replay CLI

- Chose Next Queue item 3 and completed a deterministic replay slice for longer paper validation.
- Added `kraken-scalp-replay`, an offline CLI that loads saved market snapshots from JSON, JSONL, or CSV, starts a paper scalp session in an in-memory SQLite database, runs the existing paper runner, and emits a JSON report.
- Added snapshot parsing for `timestamp`, `bid`, `ask`, `bid_size`, `ask_size`, and optional `volume_ratio`.
- Documented the replay command and updated the scalping plan to keep future work focused on historical-data adapters and multi-session summaries.
...
Files changed: `kraken_telegram_gateway/gateway/scalp_replay.py`, `pyproject.toml`, `tests/test_scalping.py`, `README.md`, `SCALPING_MODE.md`, `DEV_LOG.md`.

Tests: `python3 -m pytest tests/test_scalping.py -q` -> 7 passed. `python3 -m compileall -q kraken_telegram_gateway` -> OK. `python3 -m kraken_telegram_gateway.gateway.scalp_replay --help` -> OK. `python3 -m pytest -q` -> 128 passed, 1 known Starlette warning.

Commit local: `Add offline scalp replay CLI`.
...
- Added `/scalp_tick_kraken` and `/scalp-tick-kraken` to trigger the existing Kraken public WebSocket paper scheduler from Telegram.
- Added options `snapshots=1..10`/`snapshots_per_session=1..10` and `timeout=1..60`/`timeout_seconds=1..60`, with compact scheduler result formatting.
- Documented the command in `README.md` and `SCALPING_MODE.md`.
- Kept scalping paper-only; no live Kraken order path or live gate was changed.

Files changed: `kraken_telegram_gateway/gateway/telegram.py`, `tests/test_telegram.py`, `README.md`, `SCALPING_MODE.md`, `DEV_LOG.md`.

Tests: `python3 -m pytest tests/test_telegram.py::test_scalp_tick_kraken_runs_scheduler_from_telegram tests/test_telegram.py::test_scalp_tick_kraken_rejects_invalid_options tests/test_api.py::test_scalp_scheduler_kraken_tick_uses_market_data_provider -q` -> 3 passed. `python3 -m compileall -q kraken_telegram_gateway` -> OK. `python3 -m pytest -q` -> 123 passed, 1 Starlette/TestClient deprecation warning.
...
- Kept the existing Telegram trade preview content.
- Added a second webhook message containing only the `trade_id` after each new preview, so mobile copy no longer depends on code block copy buttons.
- Stored processed Telegram update replies as JSON lists so retry/idempotency replays both messages without creating duplicate trades.
- Updated API/Telegram tests for the two-message webhook behavior.

Files changed: `kraken_telegram_gateway/gateway/telegram.py`, `kraken_telegram_gateway/gateway/app.py`, `tests/test_telegram.py`, `tests/test_api.py`, `README.md`, `DEV_LOG.md`.

Tests: `python3 -m pytest -q` -> 106 passed, 1 Starlette/TestClient deprecation warning. `python3 -m compileall -q kraken_telegram_gateway` -> OK.
